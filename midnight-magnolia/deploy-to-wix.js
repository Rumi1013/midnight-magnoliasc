...
${colors.reset}`);
  ...
  if (!checkWixCli()) {
    console.log(
      `${colors.red}Wix CLI is not installed. Installing now...${colors.reset}`
    );
    executeCommand('npm install -g @wix/cli', 'Installing Wix CLI');

    if (!checkWixCli()) {
      console.log(
        `${colors.red}Failed to install Wix CLI. Please install it manually with 'npm install -g @wix/cli'${colors.reset}`
      );
      rl.close();
      return;
    }
  ...
  console.log(
    `${colors.blue}> Checking available Wix commands...${colors.reset}`
  );
  try {
    ...
    console.error(
      `${colors.red}Error checking Wix commands: ${error.message}${colors.reset}`
    );
  }

  // Ask which branch to deploy
  const branch = await askQuestion(
    `${colors.gold}Which branch would you like to deploy? (master/main/feature) ${colors.reset}`
  );

  // Checkout the selected branch
  if (
    !executeCommand(`git checkout ${branch}`, `Checking out ${branch} branch`)
  ) {
    rl.close();
    return;
  }
  ...
  const confirmation = await askQuestion(
    `${colors.gold}You are about to deploy the ${branch} branch to Wix. Continue? (y/n) ${colors.reset}`
  );
  ...
  console.log(
    `${colors.blue}> Skipping build step (no build script found)...${colors.reset}`
  );
  ...
  const isProduction = await askQuestion(
    `${colors.gold}Is this a production deployment? (y/n) ${colors.reset}`
  );

  // Ask for the correct deploy command
  console.log(
    `${colors.yellow}The 'wix site deploy' command is not available in your Wix CLI version.${colors.reset}`
  );
  const deployCommand = await askQuestion(
    `${colors.gold}Please enter the correct Wix deploy command (e.g., 'wix publish'): ${colors.reset}`
  );

  if (!executeCommand(deployCommand, `Deploying ${branch} to Wix`)) {
    console.log(
      `${colors.yellow}Deployment command failed. Let's try alternative approaches.${colors.reset}`
    );

    // Try alternative deployment methods
    console.log(
      `${colors.blue}Attempting alternative deployment methods...${colors.reset}`
    );
    ...
    if (executeCommand('wix publish', 'Publishing to Wix')) {
      console.log(
        `${colors.green}Successfully published to Wix using 'wix publish'${colors.reset}`
      );
    } else {
      ...
      if (executeCommand('wix deploy', 'Deploying to Wix')) {
        console.log(
          `${colors.green}Successfully deployed to Wix using 'wix deploy'${colors.reset}`
        );
      } else {
        console.log(
          `${colors.red}Automatic deployment failed. Please refer to Wix CLI documentation for the correct deployment command.${colors.reset}`
        );
        rl.close();
        return;
      }
